...
function toDomain(row: typeof characters.$inferSelect): Character {
	return {
		...
		knownSpells: (row.known_spells ?? []) as string[],
		preparedSpells: (row.prepared_spells ?? []) as string[],
		notes: row.notes ?? "",
...
export const characterRepo = {
	...
	async update(id: string, input: UpdateCharacter): Promise<Character | null> {
		...
		if (input.knownSpells !== undefined) values.known_spells = input.knownSpells;
		if (input.preparedSpells !== undefined) values.prepared_spells = input.preparedSpells;
