...
export function createCatalogueRemoteSpellClient(
	...
	return {
		async searchSpells(input) {
			try {
				const parsedInput = CatalogueRemoteSpellSearchInputSchema.parse(input);
				const query = parsedInput.query.trim().toLowerCase();
				if (query.length === 0) return [];
				return (
					await searchEntries(open5eBaseUrl, legacyBaseUrl, fetcher, parsedInput.slotLevel, query)
		...
		async getSpellDetails(spellIndex, source = "spell") {
			try {
				const index = CatalogueRemoteSpellIndexSchema.parse(spellIndex);
				const parsedSource = CatalogueRemoteSpellSourceSchema.parse(source);
				return getDetails(open5eBaseUrl, legacyBaseUrl, fetcher, index, parsedSource);
			} catch (error) {
				if (error instanceof CatalogueRemoteSpellClientError) throw error;
				throw new CatalogueRemoteSpellClientError();
			}
