...
describe("character attributes service snapshots", () => {
	it.each([
		"get",
		"update",
	] as const)("uses the current persisted level for %s responses", async (operation) => {
		...
		const service = createCharacterAttributesService({ repository: attributesRepository });
