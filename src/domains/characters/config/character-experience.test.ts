...
describe("getCharacterExperienceProgress", () => {
	it.each([
		...
		[1, 0.5],
		[1, 10_000_000],
	])("rejects invalid level %s or experience %s", (level, experience) => {
		expect(() => getCharacterExperienceProgress(level, experience)).toThrow();
	});
	...
	it.each([
		...
		[34_000, 100],
		[9_999_999, 100],
	])("clamps level-seven progress at XP %s to %s without changing the level", (experience, percent) => {
		expect(getCharacterExperienceProgress(7, experience)).toMatchObject({
			level: 7,
			progressPercent: percent,
			experienceIntoLevel: Math.max(0, experience - 23_000),
		});
	});
