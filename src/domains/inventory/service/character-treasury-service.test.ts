...
import type { CharacterTreasuryMutation, CharacterTreasuryRepository } from "../repo/index.js";
import { type CharacterTreasury, getCurrencyTotalValue } from "../types/index.js";
import { createCharacterTreasuryService, InsufficientFundsError } from "./index.js";
...
describe("createCharacterTreasuryService", () => {
	...
	it("previews insufficient spend without mutation and applies it with rollback-safe errors", async () => {
		...
		const request = { amount: { denomination: "gp" as const, amount: 1 } };

		const preview = await service.previewSpendCharacterTreasury(userId, characterId, request);
		expect(preview.preview).toMatchObject({
			canApply: false,
			error: {
		...
		await expect(
			service.spendCharacterTreasury(userId, characterId, request),
...
function fakeDependencies(initial = { cp: 0, sp: 0, gp: 0, pp: 0 }) {
	...
	const repository: CharacterTreasuryRepository = {
		...
		mutateCharacterTreasury: vi.fn(async (id: string, mutation: CharacterTreasuryMutation) => {
