...
test("mobile spells keep play actions visible and configuration recoverable", async ({
	...
	await expect(page.getByLabel("Search cantrips and features")).toHaveValue("light");
	await expect(row).toContainText("You touch one object");
	expect((await row.boundingBox())?.height).toBe(initialHeight);
	for (const width of [320, 390, 1280]) {
		await page.setViewportSize({ width, height: 900 });
		await expect(row).toBeInViewport();
		await page.screenshot({ path: testInfo.outputPath(`spell-search-${width}.png`) });
		await more.click();
		await page.screenshot({ path: testInfo.outputPath(`spell-details-${width}.png`) });
