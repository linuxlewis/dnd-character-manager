...
test("keeps failed health edits unchanged and updates only detail after explicit retry", async ({
	...
	const characterPath = `/characters/${new URL(page.url()).pathname.split("/")[2]}`;
	...
	await page.getByRole("button", { name: "Health history", exact: true }).click();
	...
	await expect(page.getByRole("button", { name: "Edit health: 10 / 10 HP" })).toBeVisible();
	await expect(page.getByLabel("Amount")).toHaveValue("3");
	...
	fail = false;
	await page.getByRole("button", { name: "Apply damage", exact: true }).click();
	await expect(page.getByRole("button", { name: "Edit health: 7 / 10 HP" })).toBeVisible();
