...
| inventory | A | A | A | A | A | A | A | Owns currency calculations, scopes, items, treasury and history. R4b uses narrow identity access with transaction-bound ownership, current-state writes and atomic history; existing catalogue/browser coverage retained. [Evidence](./domain-inventory-ownership.md) |
| characters | B | B | B | B | B | B | B | Identity, class, level and XP only; narrow access/locking and identity initialization. Creation/detail composition belongs to application. [Access evidence](./domain-character-access-creation.md) |
| health | B | B | B | B | B | B | B | Owns HP/history, absolute updates, normalization, public initialization and UI. Real transaction/rollback/transfer and browser cache failure/retry coverage. [Backend](./domain-health-composition.md), [UI](./domain-health-ui.md) |
| spellcasting | B | B | B | B | B | B | B | Owns slots/spells/defaults/history and focused modal workflows. Contended writes, bounded remote revalidation, cache origin and failure/retry covered; no general lost-response protocol. [Backend](./domain-spellcasting-backend.md), [workflows](./domain-spell-workflows.md) |

Extracted domains retain the prior B assessment; moving files or adding tests does
not alone justify A. Inventory config is now graded explicitly because it owns
currency calculation/planning previously mixed into types/services. Schema
registration is covered across domains by the [17-table registry evidence](./domain-schema-registration.md).

Application composition is tracked separately: atomic character creation,
owner-filtered relational detail, page assembly and targeted health-cache updates
have unit/integration/browser coverage. See the [module map](./domain-refactor-module-map.md)
and [final integrated acceptance](./domain-refactor-acceptance.md).
...
The final strict boundary gate has zero findings. See [evidence](./domain-health-ui.md).
...
lifecycle coverage. The character-to-inventory UI finding present at R3 was
resolved in R6; final strict lint has zero findings. See [validation evidence](./domain-schema-registration.md).
...
write adoption is verified in R4b; health and spell state-after-lock guarantees are
verified separately in R5/R7. Primitive access tests alone do not establish those
...
relocation and cache coordination are verified in R6. See [implementation and evidence](./domain-health-composition.md).
